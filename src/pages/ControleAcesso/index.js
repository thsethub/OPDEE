...
export default function ControleAcesso({ route }) {
    ...
    const fetchUsuarioLogado = async () => {
        try {
            ...
            const { data: usuarioData, error: usuarioError } = await axios.get(`${API_URL}/usuario/${deviceId}`);
    ...
    const fetchSolicitacoes = async () => {
        try {  
            setLoading(true);
            const { data: acessosData, error: acessosError } = await axios.get(`${API_URL}/acesso/ambiente/${ambienteId}`);
            ...
            const solicitacoesComNomes = await Promise.all(acessosData.map(async (solicitacao) => {
                const { data: usuarioData, error: usuarioError } = await axios.get(`${API_URL}/usuario/${solicitacao.usuarioId}`);
    ...
    const fetchNomeAmbiente = async () => {
        try {
            ...
            const { data: ambienteData, error: ambienteError } = await axios.get(`${API_URL}/ambiente/onlyOne/${ambienteId}`);
    ...
    const fetchPerfis = async () => {
        try {
            const { data: perfisData, error: perfisError } = await axios.get(`${API_URL}/perfil`);
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            const { data, error } = await axios.delete(`${API_URL}/acesso/${solicitacaoId}`);
    ...
    const handleToggleActivation = async (solicitacaoId, ativo) => {
        try{
            const { data, error } = await axios.put(`${API_URL}/acesso/${solicitacaoId}`)
            if(error){
    ...
    const handleConfirmEditPerfil = async () => {
        ...
        if (selectedPerfil) {
            try {
                const { data, error } = await axios.put(`${API_URL}/acesso/perfil/${solicitacaoToEdit.id}`, {
                ...
                <Animatable.View animation="fadeInUp" style={styles.containerForm}>
                    {loading ? (
                        ...
                        solicitacoes.map(solicitacao => (
                            <View style={styles.solicitacaoContainer} key={solicitacao.usuarioId}>
                                <View style={{ flexDirection: 'column' }}>
                                    ...
                                    <TouchableOpacity onPress={() => handleEditPerfil(solicitacao)}>
                                        <Text style={[styles.solicitacaoText, { color: "#a31821" }]}>Editar Perfil!</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => handleDelete(solicitacao.id)}>
                                        <Text style={[styles.solicitacaoText, { color: "#a31821" }]}>Excluir solicitação!</Text>
                                    </TouchableOpacity>
                                </View>
                                <Switch
