...
const Config1 = () => {
    ...
    const handleAddPerfil = async () => {
        ...
        try {
            const { data, error } = await axios.post(`${API_URL}/perfil`, currentPerfil);
            if (error) {
    ...
    const handleEditPerfil = async (id) => {
        try {
            const { data, error } = await axios.put(`${API_URL}/perfil/${id}`, currentPerfil);
            if (error) {
                console.error("Erro ao atualizar perfil: ", error.message);
                Alert.alert("Erro", `Não foi possível atualizar o perfil. Detalhes: ${error.message}`);
            } else {
                fetchPerfis();
                setModalVisible(false);
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: "OK",
                    onPress: async () => {
                        const { data: perfilData, error: perfilError } = await axios.delete(`${API_URL}/perfil/${id}`);
                        if (perfilError) {
                    ...
                    <View style={styles.modalView}>
                        ...
                        <View style={styles.buttonModal}>
                            <Button
                                ...
                                color={currentPerfil.id ? "#fd7e14" : "#28a745"}
                            />
                            <Button
                                title="Cancelar"
